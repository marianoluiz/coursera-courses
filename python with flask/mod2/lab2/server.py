...
@app.route("/exp")
def index_explicit():
  """ returns json with status code of 200 """
  res = app.make_response({ "message" : "Hello World" })
  # this is possible:
  # res = app.make_response(({"message": "Hello World"}, 200))
...
def find_by_uuid(var_name):
  ...
  if not var_name:
      return {"message": "Person is missing"}, 422
  
  for person in data:
      if var_name.lower() in person["id"].lower():
        return person
